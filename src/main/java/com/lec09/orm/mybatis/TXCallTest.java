...
public class TXCallTest {
	...
	public static void main(String[] args){
		...
		// 설정에 <bean> 등록이 되어 있는 경우 : bean의 id 또는 name으로 getBean()
		...
		// 설정에 <bean> 등록이 안되어 있는 경우 (어노테이션 기반인 경우) : 클래스로 getBean()
		if (svc!=null)
			System.out.println("svs ins. ok");
...
//		uvo.setUserId("tranc");
//		uvo.setUserPw("777");
//		int res = svc.svcUserInsert(uvo);
//		System.out.println("입력완료" + res);
		
		//--------------RuntimeException unchecked 예외발생 상황 : 롤백처리 확인--------------------
		uvo.setUserId("runtimeError");
		uvo.setUserPw("000");
		svc.svcInsertRuntimeErrorFunc(uvo);
		
		//--------------SQLException checked 예외발생 
		uvo.setUserId("errorSQL");
		uvo.setUserPw("00");
		try {
